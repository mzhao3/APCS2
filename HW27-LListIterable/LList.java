...
public class LList<T> implements List<T>
{
    ...
    class MyIterator implements Iterator<T>
  {
    ...
    public void remove()
    {
      ...
      //A: if _okToRemove is false, it means that next() has not been called. remove() should not run if next() has not been called beforehand.
      if ( ! _okToRemove )
        throw new IllegalStateException("must call next() beforehand");
	    _okToRemove = false;

	    // if your LList is size 1
      if (_head == _tail)  {
        _head = null;
        _tail = null;
      }

      //if you are at the first element in LList...
      else if ( _dummy == _head ) {
         removeFirst();
	    }
	    //if you are at the last element in LList...
      ...
      // A: You need to consider if the item being removed is the first or last element, or if the LList is of size 1.

	    //if you are in the middle of the LList;
	    else {
